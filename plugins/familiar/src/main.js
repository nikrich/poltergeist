...
import { inFailureCooldown, isRunDue, nextRunAt } from './lib/schedule.js';
...
async function sweep(trigger) {
  // Snapshot ctx: deactivate() nulls the module-level `ctx` while a run may
  // still be in flight. Using `c` (not `ctx`) for the rest of this function
  // means an in-flight run never dereferences null when it resolves after
  // deactivation.
  const c = ctx;
  ...
  running = false;
  if (ctx === null) {
    // Plugin was deactivated while this run was in flight. state.json's
    // runningSince entry goes stale and is reaped after STALE_RUN_MS on the
    // next activation; there's no ctx left to log through or notify via ipc.
    return { started: true };
  }
  ...
  if (report.rawOutput) {
    ...
    writeFileSync(join(c.dataDir, `failed-${Date.now()}.txt`), report.rawOutput);
...
function tick() {
  const state = loadState();
  if (inFailureCooldown(state, new Date())) return;
  if (isRunDue(config(), state, new Date())) void sweep('schedule');
}
