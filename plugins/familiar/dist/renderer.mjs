...
function parseOpenLoops(md) {
  ...
  for (const line of md.split("\n")) {
    ...
    const [, box, id, head, sourcePath, firstSeen, tag2] = m;
...
function renderLoop(l) {
  ...
  const tag2 = l.status === "stale" || l.status === "dismissed" ? ` {${l.status}}` : "";
  return `- [${box}] <!--id:${l.id}--> ${text}${owed} (from [source](${l.sourcePath}), first seen ${l.firstSeen})${tag2}`;
}
...
function statusLine(status) {
  ...
  return parts.join(" \xB7 ") || "No runs yet";
}
...
function mount(el, api) {
  ...
  runBtn.onmouseenter = () => {
    runBtn.style.background = neon;
    runBtn.style.color = paper;
  };
  runBtn.onmouseleave = () => {
    runBtn.style.background = "transparent";
    runBtn.style.color = neon;
  };
  ...
  function renderSettings(st) {
    ...
    form.append("Weekly on", day, "at", hour, ":00 \xB7 model", model, "\xB7 budget (chars)", budget, save);
