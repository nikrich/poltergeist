...
def test_build_digest_input_aggregates_by_context(vault: Path) -> None:
    ...
    note_a = _write_inbox_note(
        vault, "n-a.md",
        {"id": "a", "title": "Build worker", "context": "consulting",
    ...
    note_b = _write_inbox_note(
        vault, "n-b.md",
        {"id": "b", "title": "Rockets fix", "context": "work",
         "source": "claude-code", "routingMethod": "path",
    ...
    _write_audit(vault, summary_day, [
        ...
        {"ts": "2026-05-07T10:01:00Z", "event_type": "event_processed",
         ...
         "context": "consulting", "method": "path",
    ...
    assert {n.title for n in digest.notes} == {"Build worker", "Rockets fix"}
    assert set(digest.by_context.keys()) == {"consulting", "work"}
    assert digest.by_context["consulting"][0].artifact_count == 2
...
def test_render_input_for_prompt_groups_by_context(vault: Path) -> None:
    ...
    notes = [
        CapturedNote(title="A", context="consulting", source="claude-code",
                     artifact_count=2, note_path=None, ingested_at=None,
                     routing_method="path", routing_confidence=1.0),
        CapturedNote(title="B", context="work", source="claude-code",
    ...
    d = DigestInput(
        ...
        notes=notes,
        by_context={"consulting": [notes[0]], "work": [notes[1]]},
        health={"processed": 2, "failed": 0, "last_capture": "2026-05-07T11:00:00Z"},
        review_queue=[],
    )

    rendered = render_input_for_prompt(d)
    assert "Context: work" in rendered  # known contexts in canonical order
    assert "Context: consulting" in rendered
    # work comes before consulting per the configured context ordering
    assert rendered.index("Context: work") < rendered.index("Context: consulting")
    assert "2 artifact(s)" in rendered  # consulting had 2
...
def test_generate_digest_writes_to_vault(vault: Path) -> None:
    ...
    note = _write_inbox_note(
        vault, "n.md",
        {"id": "x", "title": "Real session", "context": "consulting",
         "source": "claude-code", "routingMethod": "path",
         "routingConfidence": 1.0, "ingestedAt": "2026-05-07T10:00Z"},
        "# Real session\n\n",
    )
...
def test_generate_digest_falls_back_when_llm_errors(vault: Path) -> None:
    ...
    note = _write_inbox_note(
        vault, "n.md",
        {"id": "x", "title": "S", "context": "consulting",
         "source": "claude-code", "routingMethod": "path",
...
def test_per_context_digest_only_when_threshold_met(vault: Path) -> None:
    ...
    summary_day = date(2026, 5, 7)
    # Make 6 consulting events (above the 5-event threshold)
    audit_events = []
    for i in range(6):
        note = _write_inbox_note(
            vault, f"n-{i}.md",
            {"id": f"e{i}", "title": f"Note {i}", "context": "consulting",
             "source": "claude-code", "routingMethod": "path",
             "routingConfidence": 1.0},
            "# x\n",
        )
        audit_events.append({
            ...
            "source": "claude-code", "context": "consulting", "method": "path",
            "inbox_path": str(note), "artifact_count": 0,
        })
    ...
    assert (by_ctx / "consulting-2026-05-08.md").exists()
    # No work events → no per-context file.
    assert not (by_ctx / "work-2026-05-08.md").exists()
