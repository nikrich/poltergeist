...
def test_load_cursors_empty_when_no_file(tmp_path: Path) -> None:
    state = load_cursors(tmp_path, "acme")
    assert isinstance(state, CursorState)
    assert state.channels == {}
    assert state.get("C1") is None
    assert state.path == cursor_path(tmp_path, "acme")


def test_save_and_load_roundtrip(tmp_path: Path) -> None:
    state = load_cursors(tmp_path, "acme")
    state.set("C1", last_ts="1778250000.000001", name="engineering")
    state.set("C2", last_ts="1778250001.123456", name="work-ops")
    state.save()

    reloaded = load_cursors(tmp_path, "acme")
...
def test_corrupt_file_returns_empty_state(tmp_path: Path) -> None:
    path = cursor_path(tmp_path, "acme")
...
def test_workspaces_have_separate_files(tmp_path: Path) -> None:
    a = load_cursors(tmp_path, "acme")
    a.set("C1", last_ts="111", name="x")
    a.save()
    b = load_cursors(tmp_path, "consulting")
    b.set("C1", last_ts="222", name="y")
    b.save()

    assert load_cursors(tmp_path, "acme").get("C1") == "111"
    assert load_cursors(tmp_path, "consulting").get("C1") == "222"
