...
macOS only (`skip` elsewhere), and reported first among the recorder checks. Reads `recorder.capture_backend` (`auto` by default, or `native`/`blackhole`) and reports which capture path is actually in effect. `native` uses the bundled ScreenCaptureKit helper — no ffmpeg, no BlackHole, no SwitchAudioSource, no multi-output device — and needs macOS 15+ plus Screen Recording and Microphone permission. `blackhole` is the legacy ffmpeg + BlackHole path covered by the five checks below. With the helper ready, this is `ok` reporting `native`. With `capture_backend: blackhole` explicitly configured, it's `ok` reporting the BlackHole path and the five checks below run as usual. With `auto` (the default) and the helper not ready, it's a `warn`, not a `fail` — falling back to BlackHole is a valid working state — and says why the helper isn't ready; the five BlackHole checks still run in this case since they're what's actually needed. Only `capture_backend: native` explicitly set with the helper not ready is a `fail`. The fix is manual in all three non-ok cases: if the helper binary isn't found, build/install it from source (`scripts/build-native-macos.sh --install`) — the packaged app already bundles it; if macOS is older than 15, either upgrade or set `capture_backend: blackhole`; otherwise (permissions) grant Screen Recording and Microphone access from the app's own Settings → meetings → diagnostics screen, not from Terminal, since macOS only lists an app under Screen Recording after it has attempted capture at least once. When the effective method is `native`, the five BlackHole-path checks below (`ffmpeg`, `switchaudio`, `blackhole`, `audio-device`, `audio-routing`) all report `skip` with the summary "not needed with native capture" — that's expected, not a problem to chase.

## ffmpeg

macOS only (`skip` elsewhere, and also `skip` when `capture-method` reports the effective method is `native` — see above). Confirms `ffmpeg` is on PATH — it captures BlackHole and the microphone together into the meeting WAV file, so without it recording can't start. The fix is automated, `setup deps --only ffmpeg`, which installs via Homebrew; if Homebrew itself isn't installed yet, the command stops and tells the user to install it from https://brew.sh first, which itself needs their macOS password. If it fails twice after a brew install reports success, check that a shell restart or `PATH` refresh happened, since a fresh Homebrew install doesn't always land on PATH for the current session.
...
On Windows, `capture-method`, `ffmpeg`, `switchaudio`, `blackhole`, `audio-device`, and `audio-routing` all report `skip` — they're the macOS-only pieces of the capture-method decision and the ffmpeg + BlackHole capture chain. `whisper-cli` and `whisper-model` still run and matter on Windows, but their fixes are manual (no Homebrew), pointing at `docs/install/windows.md`. In their place, `recorder-backend` runs the WASAPI backend's own preflight and is the one check that actually verifies Windows recording is ready to go.

## Recorder gotchas (macOS)

- **Permissions are attributed to whichever process attempted capture.** Grant Screen Recording and Microphone access from the app's own grant-access button, not from Terminal — macOS only lists an app under Screen Recording after that specific process has attempted capture at least once.
- **`auto` falls back silently.** With `capture_backend: auto` (the default), the app switches to the BlackHole path the moment the native helper is missing or unpermitted, with no error dialog — the `capture-method` check's `warn`, and the app's own preflight/diagnostics screen, are where the reason shows up.
- **No meeting window on screen.** If the native helper can't find a meeting-app window when a recording starts, the app offers a window picker (pick the window), "entire screen", or "audio only" — this is expected, not a failure.
