...
def _seed_two(vault):
    ...
    (vault / "20-contexts" / "work" / "notes").mkdir(parents=True, exist_ok=True)
    a = write_inbox_jot("first jot #ui", captured_at=t1)
    b = write_inbox_jot("second jot helix #idea", captured_at=t2)
...
def test_list_q_filter(tmp_vault, client, auth_headers):
    _seed_two(tmp_vault)
    resp = client.get("/v1/notes?source=manual&q=helix", headers=auth_headers)
    data = resp.json()
    assert data["total"] == 1
    assert "helix" in data["items"][0]["title"]
...
def test_list_context_filter(tmp_vault, client, auth_headers):
    a, b = _seed_two(tmp_vault)
    resp = client.get("/v1/notes?source=manual&context=work", headers=auth_headers)
    data = resp.json()
